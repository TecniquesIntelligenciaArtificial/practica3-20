...
def k_folder_cross_classification(classifier, data, target_class, transform = None, folds=10, normalize=sc, lda = lda):
  ...
  for idx, (train, test) in enumerate(kfold.split(data)):
    if transform == None:
      ...
      data[train] = normalize.fit_transform(data[train], target_class[train])
      data[test] = normalize.transform(data[test])
      dataTrain = lda.fit_transform(data[train], target_class[train])
      dataTest = lda.transform(data[test])
